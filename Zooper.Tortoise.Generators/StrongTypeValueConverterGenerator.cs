...
public class StrongTypeConverterGenerator : IIncrementalGenerator
{
	public void Initialize(IncrementalGeneratorInitializationContext context)
	{
		...
		context.RegisterSourceOutput(
				...
				source) =>
			{
				...
				// Check if EF Core is referenced
				var isEntityFrameworkCoreReferenced = IsEntityFrameworkCoreReferenced(compilation);
				...
				foreach (var typeSymbol in types.Distinct())
				{
					...
					var generatedSource = GenerateConvertersClass(
						...
						isEntityFrameworkCoreReferenced
					);
	...
	{
		// Look for the ValueConverter type from EF Core
		var valueConverterSymbol = compilation.GetTypeByMetadataName(
			"Microsoft.EntityFrameworkCore.Storage.ValueConversion.ValueConverter"
		);

		return valueConverterSymbol != null;
	}
	...
	private static string GenerateConvertersClass(
		...
		bool isEntityFrameworkCoreReferenced)
	{
		...
		sourceBuilder.AppendLine(
			...
			using Zooper.Tortoise.Generators.Attributes.Converters;
			using Zooper.Tortoise.Generators.Attributes.Helpers;
			"""
		);

		// Append EFCore only if its referenced by the consuming project
		if (isEntityFrameworkCoreReferenced)
		{
			sourceBuilder.AppendLine("using Microsoft.EntityFrameworkCore.Storage.ValueConversion;");
		}
		...
		if (generateValueConverter && isEntityFrameworkCoreReferenced)
		{
